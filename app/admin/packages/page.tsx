...
import { getPackages, createOrUpdatePackage, deletePackage } from "@/lib/api";
...
export default function AdminPackagesPage() {
  ...
  const removeHighlight = (idx: number) => {
    setHighlightsList(prev => prev.filter((_, i) => i !== idx));
  };
  ...
  const removePlace = (idx: number) => {
    setPlacesList(prev => prev.filter((_, i) => i !== idx));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        nights: currentPkg?.duration?.toLowerCase().includes("night") ? 
          parseInt(currentPkg.duration.match(/\d+/)?.at(0) || "0") : 0,
        days: currentPkg?.duration?.toLowerCase().includes("day") ? 
          parseInt(currentPkg.duration.split("/").pop()?.match(/\d+/)?.at(0) || "0") : 0,
        pricing: [
          {
            hotelCategory: "Standard Package",
            plan: "MAP Plan",
            pricePerPerson: currentPkg?.startingPrice || 0,
            recommended: true
          }
        ],
        itinerary: currentPkg?.itinerary || [
          {
            ...
            description: "Arrival and local sightseeing overview."
          }
        ]
      } as any;
      
      const saved = await createOrUpdatePackage(payload);
      if (saved) {
        loadPackages();
        setModalOpen(false);
      }
          ...
          <div className="bg-white rounded-3xl border border-border max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5 flex-1 overflow-y-auto">
              ...
              <div className="flex items-center gap-3 pt-6 border-t border-border/50 sticky bottom-0 bg-white z-10">
                <button
                  type="submit"
                  className="flex-1 py-3.5 bg-gradient-to-r from-primary to-primary-dark text-white font-semibold rounded-xl flex items-center justify-center gap-2 hover:shadow-lg active:scale-[0.98] transition-all cursor-pointer text-xs"
                >
                ...
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="px-6 py-3.5 border border-border text-ink font-semibold rounded-xl hover:bg-surface transition-colors cursor-pointer text-xs"
                >
                  Cancel
                </button>
              </div>
