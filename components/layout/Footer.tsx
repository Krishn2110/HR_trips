...
import { CONTACT, SOCIAL_LINKS, SERVICES } from "@/lib/constants";
...
const socialItems = [
  ...
  { icon: TwitterXIcon, href: SOCIAL_LINKS.twitter, label: "Twitter" },
    ...
    <footer className="bg-ink text-white/80 mt-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-sm text-white/60 leading-relaxed mb-6 max-w-xs">
              Your trusted travel partner for holiday packages, hotel bookings,
              and hospitality services across India and beyond.
            </p>
            <div className="flex items-center gap-3">
              {socialItems.map(({ icon: Icon, href, label }) => (
                <a
                  ...
                  aria-label={label}
                  className="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center hover:bg-primary hover:text-white transition-all duration-200"
                >
                  <Icon />
                </a>
              ))}
              ...
              {SERVICES.map((service) => (
                <li key={service.slug}>
                  ...
                  >
                    ...
                    <span className="group-hover:translate-x-0 -translate-x-5 transition-transform">
                      {service.name}
                    </span>
                  </Link>
                </li>
      ...
      <div className="border-t border-white/10">
        <div className="container-wide py-5 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-white/40">
            © {new Date().getFullYear()} HR Trips. All rights reserved.
          </p>
          <p className="text-xs text-white/40">
            Designed with ❤️ for incredible journeys
          </p>
        </div>
      </div>
    </footer>
